...
def solve(minterms, var_list=[]):
	if len(minterms) == 0:  # Rien à faire
...
def minterms(expression):
	variables = sorted(parse_expression(expression))
	tab = table_values(len(variables))

	mnterms = []
	for i in range(len(tab)):
		variables_values = dict(zip(variables, tab[i]))
		result = evaluate(expression, variables_values)
...
def main():
	expression = input("Entrer l'expression de la fonction: ")
	mterms = minterms(expression)
	output = solve(mterms)
	print(output)
